...
Initializer::Initializer(const Frame &ReferenceFrame, float sigma, int iterations)
{
    ...
    mMaxIterations = iterations;
     ...
     /********************* Modified Here *********************/
     mvKeysXYZBird1 = ReferenceFrame.mvKeysBirdCamXYZ;
     mvKeysXYBird1 = ReferenceFrame.mvKeysBirdBaseXY;
     mvKeysBird1 = ReferenceFrame.mvKeysBird;

     mIcp = IcpSolver(iterations);

     mRefFrame = Frame(ReferenceFrame);

     file.open("icp_2d_result.txt");
     file<<fixed;
}

/********************* Modified Here *********************/
bool Initializer::Initialize(const Frame &CurrentFrame, const vector<int> &vMatches12, const vector<int> &vMatchesBird12,
        cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    mvKeysXYZBird2 = CurrentFrame.mvKeysBirdCamXYZ;
    ...
    for(int k=0;k<vMatchesBird12.size();k++)
    {
        ...
        {
            mvMatchesBird12.push_back(make_pair(k,vMatchesBird12[k]));
        }
    }
    cout<<"Initialize with Frame "<<mRefFrame.mnId<<" and "<<mCurFrame.mnId<<endl;
                            ...
                            cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    cv::Mat R12,t12;
    float score=0;
    float sigma = mSigma*Frame::pixel2meter;
    ...
    int nGood5 = CheckRT(R3,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D5, 4.0*mSigma2, vbTriangulated5, parallax5);
    int nGood6 = CheckRT(R3,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D6, 4.0*mSigma2, vbTriangulated6, parallax6);
